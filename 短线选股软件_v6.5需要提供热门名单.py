...
"""
文件名：短线选股软件_v6.5.py
版本：v6.5（用户名单驱动候选池；涨停池逻辑与 v6.0 一致保留）
日期：2026-05-11
...
# 全局配置
# ═════════════════════════════════════════════════════════════════
VERSION = "v6.5_用户热门名单+涨停池与v6.0一致"
...
def _em_css_call_with_retry(c, codes_str: str, indicators: str, options: str) -> Any:
    ...
    for i in range(n):
        ...
        print(f"[EMQuantAPI] css 第 {i + 1}/{n} 次失败: {msg}")
        if i < n - 1:
...
class EMQuantAPIAdapter(DataSourceAdapter):
    ...
    def batch_stock_display_fields(self, codes_norm: list[str], trade_date_compact: str) -> dict[str, dict[str, str]]:
        ...
        for i in range(0, len(uniq), chunk):
            ...
            df: Optional[pd.DataFrame] = None
            for fields in ("NAME,HYBLOCK", "NAME,INDUSTRY", "NAME"):
    ...
    def get_money_flow(self, symbol: str) -> dict:
        ...
        try:
            ...
            # 尝试获取资金流指标，可能包括NETAMOUNTIN, NETAMOUNTOUT, MAINNETIN等
            tradedate = datetime.now().strftime("%Y-%m-%d")
...
def calc_sell_urgency(row, prev_row, prev2_row) -> dict:
    ...
    if vol_ratio > 5 and close < open_price: urgency += 5; reasons.append("爆量滞涨(+5)")
...
def get_operation_advice(buy_score: float, sell_urgency: int) -> str:
    if sell_urgency >= 9: return "立即卖出"
    if buy_score >= RESONANCE_BUY_THRESHOLD and sell_urgency <= RESONANCE_URGENCY_CEILING: return "谨慎持有，次日观察"
    if buy_score >= 8 and sell_urgency <= 3: return "强烈买入"
    if buy_score >= 6:
        return "中等买入（小仓）"
    elif sell_urgency >= 7:
        return "收盘前卖出"
...
def write_final_report_md(
    ...
    if df_result is None or df_result.empty:
        ...
        lines.append(f"- **强烈买入(评分≥8)**: {len(df_result[df_result['买入评分'] >= 8])} 只")
        lines.append(
            f"- **中等买入(6≤评分<8)**: {len(df_result[(df_result['买入评分'] >= 6) & (df_result['买入评分'] < 8)])} 只"
        )
        lines.append("")
        ...
        lines.append("")
        n8 = len(df_result[df_result["买入评分"] >= 8])
        n6 = len(df_result[df_result["买入评分"] >= 6])
        lines.append(f"- **评分≥8**: {n8} 只；**评分≥6**: {n6} 只（含 ≥8）")
        lines.append("")
        for title, sub in (
            ("评分≥8（前 20）", df_result[df_result["买入评分"] >= 8].head(20)),
            ("6≤评分<8（前 20）", df_result[(df_result["买入评分"] >= 6) & (df_result["买入评分"] < 8)].head(20)),
        ):
            lines.append(f"### {title}")
            lines.append("")
            cols_t = [c for c in ("代码", "名称", "买入评分", "卖出紧迫度", "操作建议", "所属行业") if c in sub.columns]
        ...
        cols = [c for c in ("代码", "名称", "买入评分", "卖出紧迫度", "操作建议", "量能状态", "所属行业") if c in sub.columns]
        if cols:
        ...
        lines.append("")
    lines.append("---")
    lines.append("*由 短线选股软件_v6.5.py 自动生成*")
    try:
...
def save_excel_with_industry_sheets(df_result: pd.DataFrame, xlsx_path: Path, run_meta: Optional[dict] = None):
    ...
    with pd.ExcelWriter(xlsx_path, engine='openpyxl') as writer:
        ...
        df_strong = df_result[df_result['买入评分'] >= 8]
        if not df_strong.empty: df_strong[valid_cols].to_excel(writer, sheet_name='强烈买入(评分≥8)', index=False)
        df_medium = df_result[(df_result['买入评分'] >= 6) & (df_result['买入评分'] < 8)]
        if not df_medium.empty: df_medium[valid_cols].to_excel(writer, sheet_name='中等买入(6≤评分<8)', index=False)
        ...
        if '所属行业' in df_result.columns:
            ...
            industry_stats = df_result.groupby('所属行业').agg({
                '代码': 'count', '买入评分': 'mean',
                '操作建议': lambda x: (x == '强烈买入').sum()
            }).rename(columns={'代码': '股票数', '买入评分': '平均评分', '操作建议': '强烈买入数'})
...
def generate_html_heatmap(
    ...
    strong_buy = len(df_result[df_result['买入评分'] >= 8])
    hold_count = len(df_result[df_result['操作建议'] == '谨慎持有，次日观察'])
    ...
    html = f"""<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8">
        ...
        <div class="cards">
          ...
          <div class="card"><div class="num" style="color:#3fb950">{strong_buy}</div><div class="label">强烈买入</div></div>
          <div class="card"><div class="num" style="color:#d29922">{hold_count}</div><div class="label">谨慎持有</div></div>
        ...
        <div class="chart-box">
          ...
          <th>买入评分</th><th>卖出紧迫度</th><th>操作建议</th><th>量能状态</th><th>止损价</th><th>止盈价</th><th>盈亏比</th><th>风险%</th><th>共振</th>
...
def build_screener_signal_row(
    ...
    if latest["turnover_rate"] > MAX_TURNOVER_RATE:
        return None
    if (latest.get("VOL_RATIO", 1) or 1) < MIN_VOL_RATIO_STRICT:
        return None
    ...
    advice = get_operation_advice(buy_result["score"], sell_result["urgency"])
    ...
    if len(df) >= 6 and df.iloc[-6]["close"] > 0:
        ...
        change_5d = round((current_price / price_5d_ago - 1) * 100, 2)
    else:
        change_5d = 0
    ...
    return {
        ...
        "量比": round(float(latest.get("VOL_RATIO", 1) or 1), 2),
        "量能状态": vol_status,
        "买入评分": buy_result["score"],
        "卖出紧迫度": sell_result["urgency"],
        "操作建议": advice,
        "止损价": sltp["stop_loss"],
...
def run_screener(data_source: DataSourceAdapter, target_date: str = None):
    ...
    print(
        "[规则] v6.5 扫描候选仅来自用户名单文件（见 HOT_LIST_FILE / STOCK_HOT_LIST_FILE），"
    ...
    if not display_df.empty:
        for _, row in display_df.iterrows():
            ...
            print(f"  {row['代码']} {row['名称']:<8} [{row.get('所属行业', '')}] "
                  f"评分{row['买入评分']:>5.1f} | 紧迫{row['卖出紧迫度']:>4.1f} | "
    ...
    print(f"  强烈买入(≥8)：{len(df_result[df_result['买入评分'] >= 8])}")
    print(f"  中等买入(6~8)：{len(df_result[(df_result['买入评分'] >= 6) & (df_result['买入评分'] < 8)])}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="短线选股 v6.5（用户名单候选；涨停池/K 线逻辑同 v6.0）")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="v6.5：名单文件路径，或「热门股名单」目录（则读目录内 默认热门名单.txt）；覆盖 HOT_LIST_FILE / STOCK_HOT_LIST_FILE",
    )
    parser.add_argument(
        "--top-hot",
        type=int,
    ...
    if args.top_hot is not None:
        print("[提示] v6.5 忽略 --top-hot：候选股票以名单文件为准（HOT_LIST_FILE）")
