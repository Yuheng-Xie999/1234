# -*- coding: utf-8 -*-
"""
文件名：短线选股软件_v6.6.1.py
版本：v6.6.1（保留V6.5东财K线；修复量比NaN；增强CSS诊断；保存完整控制台日志）
日期：2026-05-11
...
# 全局配置
# ═════════════════════════════════════════════════════════════════
VERSION = "v6.6.1_保留东财K线+修复量比NaN+保存控制台日志"
...
DEFAULT_CONFIG = {
    ...
    # 东财 css 批量名称/行业每批代码数；过大时部分账号/终端会失败，v6.6.1 默认降到 80
    "EM_CSS_BATCH_CODES": 80,
    ...
    def batch_stock_display_fields(self, codes_norm: list[str], trade_date_compact: str) -> dict[str, dict[str, str]]:
        ...
        for c in codes_norm:
            ...
            if c6 not in seen:
                seen.add(c6)
                uniq.append(c6)
        chunk = max(20, min(120, int(config.get("EM_CSS_BATCH_CODES", 80) or 80)))
...
def calc_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    for _col in ("open", "high", "low", "close", "volume", "amount", "turnover_rate", "pct_change"):
...
def build_screener_signal_row(
    ...
    # v6.6.1：量比缺失时不误当 1 或“正常”，保留样本并在结果中标注“量能缺失”。
...
def run_screener(data_source: DataSourceAdapter, target_date: str = None):
    ...
    print(
        "[规则] v6.6.1 扫描候选仅来自用户名单文件（见 HOT_LIST_FILE / STOCK_HOT_LIST_FILE），"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="短线选股 v6.6.1（保留东财K线；修复量比NaN；保存控制台日志）")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="v6.6.1：名单文件路径，或「热门股名单」目录（则读目录内 默认热门名单.txt）；覆盖 HOT_LIST_FILE / STOCK_HOT_LIST_FILE",
    )
    parser.add_argument(
        "--top-hot",
        type=int,
    ...
    if args.top_hot is not None:
        print("[提示] v6.6.1 忽略 --top-hot：候选股票以名单文件为准（HOT_LIST_FILE）")
