# -*- coding: utf-8 -*-
"""
文件名：短线选股软件_v6.7.3.py
版本：v6.7.3（真实涨停阈值；风险扣分不归零；涨停日 BIAS/CCI/WR/过热共振降权；高风险拆分）
日期：2026-05-16
基于：短线选股软件_v6.7.2.py

【v6.7.3 变更摘要】
  - 涨停判定统一走 get_limit_up_threshold（主板/创业板/科创板/北交所），东财涨停池筛选与次日风险/接力逻辑不再写死 9.5%/9.9%。
  - 风险惩罚后买入评分允许为负，不再 max(0, …) 截断；原始买入评分同样保留负分。
  - 涨停当日：BIAS 偏高扣分、CCI/WR 过热、多指标过热共振在次日风险中按 0.5 倍计权（延续 v6.7.2 的 CCI/WR，补齐 BIAS 与共振）。
  - 操作建议「高风险回避」拆为「接力失败回避」（含接力失败类风险原因）与「高波动强势股」（其余极高风险）。
...
# 全局配置
# ═════════════════════════════════════════════════════════════════
VERSION = "v6.7.3_真实涨停阈值+风险扣分不归零+涨停日共振降权+高风险拆分"
...
LIMIT_UP_PCT_SLACK = 0.5


def is_real_limit_up(pct: float, code: str, slack: float = LIMIT_UP_PCT_SLACK) -> bool:
    """按板块真实涨停幅度判定（阈值 - slack，与连板 K 线回溯一致）。"""
    return _safe_float(pct, 0) >= get_limit_up_threshold(code) - slack
...
class EMQuantAPIAdapter(DataSourceAdapter):
    ...
    def get_limit_up_pool(self, date_str: str, trade_date: Optional[str] = None) -> pd.DataFrame:
        ...
        try:
            ...
            # 筛选涨停股票：按各代码真实涨停阈值（非写死 9.9%）
            code_col = df["CODE"].astype(str).map(
                lambda x: normalize_stock_code(x.split(".")[0] if "." in str(x) else x)
            )
            zt_thr = code_col.map(lambda c: get_limit_up_threshold(c) - LIMIT_UP_PCT_SLACK)
            limit_up = df[df["pct_change"] >= zt_thr].copy()
...
# 5. 买入评分 / 卖出紧迫度 / 操作建议
# ═════════════════════════════════════════════════════════════════
def calc_buy_score(row, prev_row, is_limit_up_yesterday, zt_strength: float = 0.0, extra_signals: dict = None, code: str = "") -> dict:
    score = 0.0;
    details = []
    ...
    today_pct = _safe_float(row.get('pct_change', 0), 0)
    is_today_limit_up = is_real_limit_up(today_pct, code) if code else today_pct >= (9.8 - LIMIT_UP_PCT_SLACK)
    bias_penalty_factor = 0.5 if is_today_limit_up else 1.0
    if bias5 > 12:
        d = 2.0 * bias_penalty_factor
        score -= d
        details.append(f"BIAS5={bias5:.1f}%严重偏离(-{d:g})" + ("（涨停日降权）" if is_today_limit_up else ""))
    elif bias5 > 8:
        d = 1.0 * bias_penalty_factor
        score -= d
        details.append(f"BIAS5={bias5:.1f}%明显偏高(-{d:g})" + ("（涨停日降权）" if is_today_limit_up else ""))
    elif bias5 > 6:
        d = 0.5 * bias_penalty_factor
        score -= d
        details.append(f"BIAS5={bias5:.1f}%轻度偏高(-{d:g})" + ("（涨停日降权）" if is_today_limit_up else ""))
    ...
    return {'score': round(score, 1), 'details': "; ".join(details) if details else "无得分"}


def calc_next_day_risk(latest, prev, extra_signals=None, code: str = "") -> dict:
    ...
    is_limit_up = is_real_limit_up(pct, code_s) if code_s else pct >= (9.8 - LIMIT_UP_PCT_SLACK)
    prev_limit_up = is_real_limit_up(prev_pct, code_s) if code_s else prev_pct >= (9.8 - LIMIT_UP_PCT_SLACK)
    zt_down_factor = 0.5 if is_limit_up else 1.0
    ...
    # 3. BIAS 偏高（v6.7.3：涨停日降权）
    bias_factor = zt_down_factor
    if bias5 >= 12:
        risk += 4 * bias_factor; penalty += 2.0 * bias_factor
        reasons.append(f"BIAS5={bias5:.1f}%严重偏离" + ("（涨停日降权）" if is_limit_up else ""))
    elif bias5 >= 8:
        risk += 3 * bias_factor; penalty += 1.5 * bias_factor
        reasons.append(f"BIAS5={bias5:.1f}%明显偏高" + ("（涨停日降权）" if is_limit_up else ""))
    elif bias5 >= 6 and pct >= 5:
        risk += 2 * bias_factor; penalty += 1.0 * bias_factor
        reasons.append(f"BIAS5={bias5:.1f}%轻度偏高且当日上涨" + ("（涨停日降权）" if is_limit_up else ""))

    # 4. CCI 过热（涨停日降权）
    cci_factor = zt_down_factor
    if cci >= 200:
    ...
    if bias5 >= 6:
        hot_count += 1
    hot_factor = zt_down_factor
    if hot_count >= 3:
        risk += 5 * hot_factor; penalty += 2.5 * hot_factor
        reasons.append("多指标过热共振" + ("（涨停日降权）" if is_limit_up else ""))
    elif hot_count == 2:
        risk += 3 * hot_factor; penalty += 1.5 * hot_factor
...
def get_operation_advice(buy_score: float, sell_urgency: float, next_day_risk: float = 0, risk_reasons: str = "") -> str:
    relay_fail = "接力失败" in (risk_reasons or "")
    if sell_urgency >= 9 or next_day_risk >= 9:
        return "接力失败回避" if relay_fail else "高波动强势股"
...
def write_final_report_md(
    ...
    if df_result is None or df_result.empty:
        ...
        lines.append("")
    lines.append("---")
    lines.append("*由 短线选股软件_v6.7.2.py 自动生成*")
    try:
...
def build_screener_signal_row(
    ...
    next_risk = calc_next_day_risk(latest, prev, extra_signals, code=code)
    buy_result = calc_buy_score(latest, prev, is_zt, zt_strength, extra_signals, code=code)
    raw_buy_score = buy_result["score"]
    adjusted_buy_score = round(raw_buy_score - next_risk["penalty"], 1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="短线选股 v6.7.3（真实涨停阈值+风险扣分不归零+涨停日共振降权+高风险拆分）")
