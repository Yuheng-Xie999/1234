...
# 全局配置
# ═════════════════════════════════════════════════════════════════
VERSION = "v6.7.4_修复操作建议顺序+收紧高波动条件+数据完整警告+数据源降配"
...
DEFAULT_CONFIG = {
    ...
    # v6.7.4：降线程防数据源限流（东财 insufficient user access），加大超时等响应
    "THREAD_WORKERS": 3, "BATCH_SIZE": 20, "ERROR_THRESHOLD": 0.35,
    ...
    # 失败率超 ERROR_THRESHOLD 时是否终止整次扫描（v6.7.4：默认开启，数据源不稳定时避免大量错误扫描）
    "ERROR_ABORT_ON_HIGH_FAILURE_RATE": True,
    ...
    # 多数据源并行查询超时（秒），v6.7.4：从20增至45秒，应对AKShare断连等不稳定情况
    "MULTI_SOURCE_TIMEOUT_SEC": 45,
...
def is_real_limit_up_by_code(code: str, pct: float, tolerance: float = 0.5) -> bool:
    """按股票所属板块判断是否真实涨停/接近涨停，避免把创业板/科创板 +10% 误判为涨停。"""
    try:
        threshold = get_limit_up_threshold(code)
        return float(pct) >= threshold - float(tolerance)
    except Exception:
        return float(pct or 0) >= 9.5
...
def calc_buy_score(row, prev_row, is_limit_up_yesterday, zt_strength: float = 0.0, extra_signals: dict = None) -> dict:
    ...
    bias_penalty_factor = 0.4 if bool(extra_signals.get("is_real_limit_up_today", False)) else 1.0
    if bias5 > 12:
        delta = -2.0 * bias_penalty_factor
        score += delta
        details.append(f"BIAS5={bias5:.1f}%严重偏离({delta:+.1f})")
    elif bias5 > 8:
        delta = -1.0 * bias_penalty_factor
        score += delta
        details.append(f"BIAS5={bias5:.1f}%明显偏高({delta:+.1f})")
    elif bias5 > 6:
        delta = -0.5 * bias_penalty_factor
        score += delta
        details.append(f"BIAS5={bias5:.1f}%轻度偏高({delta:+.1f})")
    ...
    if profit_ratio > 80:
        score += 0.3; details.append(f"获利盘占比{profit_ratio:.0f}%(+0.3)")
    elif profit_ratio < 30:
        score -= 1.0; details.append(f"获利盘占比{profit_ratio:.0f}%(-1)")
...
def calc_next_day_risk(code: str, latest, prev, extra_signals=None) -> dict:
    """v6.7.3：计算次日风险分，用于调整买入评分。返回 {score, penalty, reasons, relay_fail_risk, high_volatility_strong, is_real_limit_up}。"""
    ...
    is_limit_up = is_real_limit_up_by_code(code, pct)
    prev_limit_up = is_real_limit_up_by_code(code, prev_pct)

    # 1. 昨日涨停后今日转弱
    if prev_limit_up and pct <= -5:
        risk += 8; penalty += 4.0
        relay_fail_risk += 8
        reasons.append("昨日涨停后今日大跌，接力失败/资金兑现")
    elif prev_limit_up and pct < 0:
        risk += 5; penalty += 2.5
        relay_fail_risk += 5
        reasons.append("昨日涨停后今日收跌，接力失败风险")

    # 2. 当日涨幅过大（v6.7.3：按真实涨停阈值判断，量能分级）
    if is_limit_up:
        if not pd.isna(vol_ratio) and 1.2 <= vol_ratio <= 3.0:
            risk += 3; penalty += 1.5
            reasons.append("放量换手涨停，次日有分歧但强势仍在")
        elif not pd.isna(vol_ratio) and vol_ratio > 3.0:
            risk += 4; penalty += 2.0
            reasons.append("爆量涨停，次日分歧风险较高")
        else:
            risk += 5; penalty += 2.5
            reasons.append("缩量/一字涨停，次日开板兑现风险")
    elif pct >= 7:
        risk += 4; penalty += 2.0
    ...
    # 3. CCI 过热（涨停日降权）
    cci_factor = 0.5 if is_limit_up else 1.0
    if cci >= 200:
        risk += 4 * cci_factor; penalty += 2.0 * cci_factor
        reasons.append("CCI极度过热" + ("（涨停日降权）" if is_limit_up else ""))
    elif cci >= 150:
        risk += 3 * cci_factor; penalty += 2.0 * cci_factor
        reasons.append("CCI过热" + ("（涨停日降权）" if is_limit_up else ""))
    elif cci >= 120 and pct >= 5:
        risk += 2 * cci_factor; penalty += 1.0 * cci_factor
        reasons.append("CCI偏热且当日上涨" + ("（涨停日降权）" if is_limit_up else ""))
    ...
    # 5. WR 过热（涨停日降权）
    wr_factor = 0.5 if is_limit_up else 1.0
    if wr >= -3 and pct >= 5:
        risk += 3 * wr_factor; penalty += 1.5 * wr_factor
        reasons.append("WR接近强势极值，短线过热" + ("（涨停日降权）" if is_limit_up else ""))
    elif wr >= -10 and pct >= 5:
        risk += 2 * wr_factor; penalty += 1.0 * wr_factor
        reasons.append("WR偏热" + ("（涨停日降权）" if is_limit_up else ""))

    # 6. 获利盘过高
    if profit_ratio >= 98 and pct >= 5:
        risk += 3; penalty += 1.5
        reasons.append("获利盘极高且当日大涨，兑现压力")
    elif profit_ratio >= 95 and pct >= 5:
        risk += 2; penalty += 1.0
        reasons.append("获利盘高且当日上涨")
    elif profit_ratio >= 95:
        risk += 1; penalty += 0.5
    ...
    if bias5 >= 6:
        hot_count += 1
    overheat_factor = 0.5 if is_limit_up else 1.0
    if hot_count >= 3:
        risk += 5 * overheat_factor
        penalty += 2.5 * overheat_factor
        reasons.append("多指标过热共振" + ("（涨停日降权）" if is_limit_up else ""))
    elif hot_count == 2:
        risk += 3 * overheat_factor
        penalty += 1.5 * overheat_factor
        reasons.append("双指标过热" + ("（涨停日降权）" if is_limit_up else ""))

    # v6.7.4：收紧高波动强势条件，防止晶科科技等普通+5%被误标
    high_volatility_strong = bool(
        relay_fail_risk < 7 and (
            is_limit_up or (
                pct >= 7
                and (not pd.isna(vol_ratio) and vol_ratio >= 1.5)
                and (rsi6 >= 65 or cci >= 120 or wr >= -10)
            )
        )
    )
    return {
        "score": round(min(risk, 10), 1),
        "penalty": round(min(penalty, 4), 1),
        "reasons": "; ".join(reasons) if reasons else "无明显次日风险",
        "relay_fail_risk": round(min(relay_fail_risk, 10), 1),
        "high_volatility_strong": high_volatility_strong,
        "is_real_limit_up": bool(is_limit_up),
    }
...
def get_operation_advice(
    buy_score: float,
    sell_urgency: float,
    next_day_risk: float = 0,
    relay_fail_risk: float = 0,
    high_volatility_strong: bool = False,
) -> str:
    # v6.7.4：修复判断顺序，高波动强势股优先于普通 sell_urgency>=9 处理
    if relay_fail_risk >= 7:
        return "接力失败回避"
    if next_day_risk >= 9 and high_volatility_strong:
        if sell_urgency >= 9:
            return "高波动强势股，只看盘中确认（卖压高）"
        return "高波动强势股，只看盘中确认"
...
def write_final_report_md(
    ...
    lines.append("")
    # v6.7.4：数据完整性警告
    dw = run_meta.get("数据完整性警告") if run_meta else None
    if dw:
    ...
    if df_result is None or df_result.empty:
        ...
        # v6.7 风控分类（字段存在性保护）
        if "次日风险分" in df_result.columns and "原始买入评分" in df_result.columns:
            ...
            downgrade_strong = len(df_result[(df_result['原始买入评分'] >= 5.5) & (df_result['次日风险分'] > 4)])
            ...
            lines.append(f"- **强势降级股(原始≥5.5 且 风险>4)**: {downgrade_strong} 只")
        else:
            lines.append("- **风控分类**: （缺少次日风险分/原始买入评分字段，跳过）")
        lines.append("")
        ...
        if "次日风险分" in df_result.columns and "原始买入评分" in df_result.columns:
            ...
            n_downgrade = len(df_result[(df_result["原始买入评分"] >= 5.5) & (df_result["次日风险分"] > 4)])
            lines.append(f"- **高评分低风险(评分≥8 且 风险≤3)**: {n_hl} 只；**中等关注(评分≥6 且 风险≤4)**: {n_mid} 只；**高风险强势股(原始≥6 但 风险>4)**: {n_risk} 只；**强势降级股(原始≥5.5 且 风险>4)**: {n_downgrade} 只")
        lines.append("")
        if "次日风险分" in df_result.columns and "原始买入评分" in df_result.columns:
            risk_tables = (
                ("高评分低风险（前 20）", df_result[(df_result["买入评分"] >= 8) & (df_result["次日风险分"] <= 3)].head(20),
                 ("代码", "名称", "买入评分", "次日风险分", "原始买入评分", "卖出紧迫度", "操作建议", "所属行业")),
                ("中等关注（前 20）", df_result[(df_result["买入评分"] >= 6) & (df_result["次日风险分"] <= 4)].head(20),
                 ("代码", "名称", "买入评分", "次日风险分", "原始买入评分", "卖出紧迫度", "操作建议", "所属行业")),
                ("高风险强势股（前 20）", df_result[(df_result["原始买入评分"] >= 6) & (df_result["次日风险分"] > 4)].head(20),
                 ("代码", "名称", "原始买入评分", "买入评分", "次日风险分", "次日风险原因", "操作建议")),
                ("强势降级股（原始≥5.5 且 风险>4，前 20）", df_result[(df_result["原始买入评分"] >= 5.5) & (df_result["次日风险分"] > 4)].head(20),
                 ("代码", "名称", "原始买入评分", "买入评分", "次日风险分", "次日风险原因", "操作建议")),
            )
        else:
            risk_tables = ()
        for title, sub, risk_cols in risk_tables:
            lines.append(f"### {title}")
            lines.append("")
            cols_t = [c for c in risk_cols if c in sub.columns]
            if cols_t and not sub.empty:
                lines.append("| " + " | ".join(cols_t) + " |")
                lines.append("| " + " | ".join(["---"] * len(cols_t)) + " |")
                for _, row in sub.iterrows():
                    cells = [_md_escape_cell(row.get(c), 100 if c == "次日风险原因" else 20) for c in cols_t]
        ...
        cols = [c for c in ("代码", "名称", "买入评分", "次日风险分", "原始买入评分", "高波动强势", "真实涨停", "卖出紧迫度", "操作建议", "量能状态", "所属行业") if c in sub.columns]
        if cols:
        ...
        lines.append("")
    lines.append("---")
    lines.append("*由 短线选股软件_v6.7.3.py 自动生成*")
    try:
...
def save_excel_with_industry_sheets(df_result: pd.DataFrame, xlsx_path: Path, run_meta: Optional[dict] = None):
    ...
    with pd.ExcelWriter(xlsx_path, engine='openpyxl') as writer:
        ...
        if "次日风险分" in df_result.columns and "原始买入评分" in df_result.columns:
            ...
            df_downgrade = df_result[(df_result['原始买入评分'] >= 5.5) & (df_result['次日风险分'] > 4)]
            if not df_downgrade.empty: df_downgrade[valid_cols].to_excel(writer, sheet_name='强势降级股(原始≥5.5风险>4)', index=False)
...
def generate_html_heatmap(
    ...
    strong_buy = len(df_result[(df_result['买入评分'] >= 8) & (df_result['次日风险分'] <= 3)]) if "次日风险分" in df_result.columns else 0
    ...
    top30_rows = ""
    for i, (_, row) in enumerate(top_chart.iterrows(), 1):
        ...
        top30_rows += f"""<tr>
            <td style="color:#8b949e">{i}</td><td>{row.get('代码', '')}</td><td>{disp_name}</td><td>{safe_fmt(row.get('最新价', '-'), '.2f')}</td>
    ...
    html = f"""<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8">
        ...
        <div class="chart-box">
          <div class="table-wrap"><table><thead><tr><th>#</th><th>代码</th><th>名称</th><th>最新价</th><th>涨跌幅</th>
...
def build_screener_signal_row(
    ...
    # v6.7.3：标记真实涨停，供 calc_buy_score 中 BIAS5 降权使用
    extra_signals["is_real_limit_up_today"] = is_real_limit_up_by_code(
        code,
        latest.get("pct_change", 0) or 0
    )

    # v6.7：先算次日风险，再算原始买入评分，用风险惩罚调整
    next_risk = calc_next_day_risk(code, latest, prev, extra_signals)
    buy_result = calc_buy_score(latest, prev, is_zt, zt_strength, extra_signals)
    raw_buy_score = buy_result["score"]
    risk_penalty = next_risk["penalty"]
    if risk_penalty >= 4 and raw_buy_score >= 2.0:
        adjusted_buy_score = round(raw_buy_score * 0.35, 1)
    elif risk_penalty >= 4:
        adjusted_buy_score = max(0.5, round(raw_buy_score - risk_penalty, 1))
    else:
        adjusted_buy_score = max(0, round(raw_buy_score - risk_penalty, 1))
    ...
    advice = get_operation_advice(
        adjusted_buy_score,
        adjusted_sell_urgency,
        next_risk["score"],
        next_risk.get("relay_fail_risk", 0),
        next_risk.get("high_volatility_strong", False),
    )
    ...
    return {
        ...
        "接力失败风险": next_risk.get("relay_fail_risk", 0),
        "高波动强势": "是" if next_risk.get("high_volatility_strong", False) else "否",
        "真实涨停": "是" if next_risk.get("is_real_limit_up", False) else "否",
...
def run_screener(data_source: DataSourceAdapter, target_date: str = None):
    ...
    # v6.7.4：数据完整性校验，覆盖率不足时强提醒
    coverage = len(df_result) / max(1, total)
    if coverage < 0.95:
        run_meta["数据完整性警告"] = (
            f"本轮有效输出 {len(df_result)}/{total}，覆盖率 {coverage:.1%}，"
            "K线缺失较多，结果不适合作正式对比。"
        )
    ...
    # v6.7.2：新增强势降级榜
    downgrade_df = df_result[
        (df_result["原始买入评分"] >= 5.5) &
        (df_result["次日风险分"] > 4)
    ].sort_values(["次日风险分", "原始买入评分"], ascending=[False, False]).head(20)
    print(f"\n{'=' * 70}")
    print(f"[高风险降级榜] 原始评分>=5.5 且 风险>4")
    print(f"{'=' * 70}")
    if downgrade_df.empty:
        ...
        for _, row in downgrade_df.iterrows():
            print(
                f"  {row['代码']} {row['名称']:<8} "
                f"原始{row['原始买入评分']:.1f} -> 调整{row['买入评分']:.1f} | "
                f"风险{row['次日风险分']:.1f} | {row['操作建议']} | "
                f"{row.get('次日风险原因', '')}"
            )
    ...
    if "次日风险分" in df_result.columns and "原始买入评分" in df_result.columns:
        ...
        # v6.7.2：新增强势降级股统计
        downgrade_strong = len(df_result[(df_result['原始买入评分'] >= 5.5) & (df_result['次日风险分'] > 4)])
        print(f"  强势降级股(原始≥5.5且风险>4)：{downgrade_strong}")
    else:
        print(f"  风控分类统计：（缺少次日风险分/原始买入评分字段，跳过）")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="短线选股 v6.7.1（风控评分系统+次日风险评估+利润盘阈值修复；保留东财K线+修复VOL量比兜底+修复920CSS后缀）")
